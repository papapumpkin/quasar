// Package github implements a read-only GitHub Issues TicketSource backed by
// the `gh` CLI. It is the only adapter Quasar ships with at launch; other
// trackers (Jira, Linear, …) follow the same shape in their own sub-packages.
//
...
//   - github.go the Source adapter wiring those together + registry init()
//
// All git operations are intentionally absent here: vanilla `git` belongs in
// internal/gitops/, and `gh` is used ONLY for ticket reading.
package github
...
func ParseSourceID(input, defaultRepo string) (repo string, number int, err error) {
	...
	if i := strings.Index(input, "#"); i != -1 {
		...
		if repo == "" {
			return "", 0, fmt.Errorf("github: ambiguous source id %q: use %q or a bare number with [integrations.github].repo configured", input, "owner/repo#"+numStr)
		}
		if !validRepo(repo) {
...
// integrations.Comment values, preserving gh's source ordering.
func parseComments(data []byte) ([]integrations.Comment, error) {
	...
	out := make([]integrations.Comment, 0, len(env.Comments))
	for _, c := range env.Comments {
		out = append(out, integrations.Comment{
