...
func TestOperatorNamesRegistered(t *testing.T) {
	got := OperatorNames()
	want := []string{"commit", "fail_run", "fulfill_entanglements", "master_review_decision", "merge_attempt", "notify_human", "persist_phases", "render_fix_prompt", "render_seed_prompt", "reviewer_decision", "verify_build", "verify_lint", "verify_test"}
