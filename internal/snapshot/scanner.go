...
func (s *Scanner) Scan(ctx context.Context, workDir string) (string, error) {
	...
	// Read convention files (only git-tracked ones).
	conventions := readConventionFile(workDir, root)
...
// readConventionFile reads the first git-tracked convention file found in the
// work directory. Only files present in root.children (i.e., tracked by git)
// are considered, ensuring deterministic output.
func readConventionFile(workDir string, root *dirNode) string {
