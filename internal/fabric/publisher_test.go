...
// mockFabric records calls for testing. It implements the Fabric interface.
...
func newMockFabric() *mockFabric {
	return &mockFabric{claims: make(map[string]string)}
}

func (m *mockFabric) SetPhaseState(_ context.Context, _, _ string) error { return nil }
func (m *mockFabric) GetPhaseState(_ context.Context, _ string) (string, error) {
	return "", nil
}

func (m *mockFabric) PublishEntanglement(_ context.Context, c Entanglement) error {
	m.entanglements = append(m.entanglements, c)
	return m.publishErr
}

func (m *mockFabric) PublishEntanglements(_ context.Context, cs []Entanglement) error {
	m.entanglements = append(m.entanglements, cs...)
	return m.publishErr
}

func (m *mockFabric) EntanglementsFor(_ context.Context, _ string) ([]Entanglement, error) {
	return nil, nil
}
func (m *mockFabric) AllEntanglements(_ context.Context) ([]Entanglement, error) { return nil, nil }

func (m *mockFabric) ClaimFile(_ context.Context, fp, owner string) error {
	if m.claimErr != nil {
		return m.claimErr
	}
	m.claims[fp] = owner
...
`
		...
		mb := newMockFabric()
		pub := &Publisher{Fabric: mb, WorkDir: dir}
		...
		// Build a map of entanglement kind+name for easy checking.
		got := make(map[string]string) // "kind:name" -> signature
		for _, c := range mb.entanglements {
			key := c.Kind + ":" + c.Name
			got[key] = c.Signature
		}

		// Verify expected entanglements.
		wantKeys := []string{
		...
		for _, key := range wantKeys {
			if _, ok := got[key]; !ok {
				t.Errorf("missing entanglement %q; have: %v", key, keys(got))
			}
		}
		...
		for _, key := range unwantedKeys {
			if _, ok := got[key]; ok {
				t.Errorf("unexpected entanglement %q should not be extracted", key)
			}
		}

		// Verify all entanglements have correct producer and status.
		for _, c := range mb.entanglements {
			if c.Producer != "phase-1" {
				t.Errorf("entanglement %s producer = %q, want %q", c.Name, c.Producer, "phase-1")
			}
			if c.Status != StatusFulfilled {
				t.Errorf("entanglement %s status = %q, want %q", c.Name, c.Status, StatusFulfilled)
			}
		}
	})

	t.Run("non-Go files produce file entanglements only", func(t *testing.T) {
		t.Parallel()
		...
		if len(mb.entanglements) != 1 {
			t.Fatalf("got %d entanglements, want 1", len(mb.entanglements))
		}
		c := mb.entanglements[0]
		if c.Kind != KindFile {
	...
	t.Run("skips test files", func(t *testing.T) {
		...
		// Create a test file — should only get a file entanglement, not symbols.
		testSrc := `package foo
...
`
		...
		// Only a file entanglement should exist.
		if len(mb.entanglements) != 1 {
			t.Fatalf("got %d entanglements, want 1 (file only)", len(mb.entanglements))
		}
		if mb.entanglements[0].Kind != KindFile {
			t.Errorf("kind = %q, want %q", mb.entanglements[0].Kind, KindFile)
		}
	})
...
`
		...
		mb := newMockFabric()
		pub := &Publisher{Fabric: mb, WorkDir: dir, Logger: &logBuf}
		...
		// Should still produce a file entanglement.
		if len(mb.entanglements) != 1 {
			t.Fatalf("got %d entanglements, want 1 (file only)", len(mb.entanglements))
		}
		if mb.entanglements[0].Kind != KindFile {
	...
	t.Run("no changes yields no entanglements", func(t *testing.T) {
		t.Parallel()
		...
		if len(mb.entanglements) != 0 {
			t.Errorf("got %d entanglements, want 0", len(mb.entanglements))
		}
	})
}
...
`
		...
		pub := &Publisher{}
		entanglements, err := pub.extractGoSymbols(path)
		if err != nil {
			t.Fatalf("extractGoSymbols: %v", err)
		}

		if len(entanglements) != 1 {
			t.Fatalf("got %d entanglements, want 1", len(entanglements))
		}
		c := entanglements[0]
...
`
		...
		got := make(map[string]Entanglement)
		for _, c := range entanglements {
			got[c.Kind+":"+c.Name] = c
		}
...
func TestPackageFromPath(t *testing.T) {
	...
	tests := []struct {
		...
		{"internal/fabric/publisher.go", "fabric"},
		{"cmd/root.go", "cmd"},
		{"main.go", ""},
		{"a/b/c/deep.go", "c"},
	}
...
// keys2 returns sorted keys from an entanglement map.
func keys2(m map[string]Entanglement) []string {
	var ks []string
	for k := range m {
		ks = append(ks, k)
	}
