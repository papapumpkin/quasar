...
// It builds a prompt containing the phase body and rendered board snapshot,
...
// Poll checks whether phaseID has enough context from the board to proceed.
// It constructs a prompt with the phase body and board snapshot, invokes the
...
// buildPollPrompt constructs the LLM prompt for evaluating board readiness.
func buildPollPrompt(phaseBody string, snap BoardSnapshot) string {
	...
	b.WriteString("\n\n")
	b.WriteString("## Available Context (Board State)\n")
	b.WriteString(RenderSnapshot(snap))
	b.WriteString("\n")
	b.WriteString("## Instructions\n")
	b.WriteString("Based on the task description and the available contracts/interfaces on the board, determine if you can proceed.\n\n")
	...
	b.WriteString("CONFLICT — A contract on the board contradicts another contract, or a file you need to modify is actively claimed by a running phase. ")
