...
// EscalationHandler manages interactive escalation dialogues for blocked
// phases. It wraps a dialogue.Opener and translates human responses into
...
// Run opens an interactive dialogue for a blocked phase and blocks until
...
func (h *EscalationHandler) Run(ctx context.Context, phaseID string, bp *fabric.BlockedPhase, maxRetries int) EscalationResult {
	...
	if err != nil {
		fmt.Fprintf(h.logger(), "warning: failed to open escalation dialogue for %q: %v\n", phaseID, err)
...
func (h *EscalationHandler) open(ctx context.Context, phaseID string, bp *fabric.BlockedPhase, maxRetries int) (dialogue.Session, error) {
	return h.Dialogue.Open(ctx, dialogue.Request{
		PhaseID: phaseID,
...
func (h *EscalationHandler) sendIntro(ctx context.Context, sess dialogue.Session, phaseID string, bp *fabric.BlockedPhase) {
...
func (h *EscalationHandler) conversationLoop(ctx context.Context, sess dialogue.Session) EscalationResult {
	for {
...
func (h *EscalationHandler) sendResolution(ctx context.Context, sess dialogue.Session, result EscalationResult) {
	switch result {
