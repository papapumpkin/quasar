...
func TestLLMPollerPoll(t *testing.T) {
	...
	t.Run("need_info with bullets", func(t *testing.T) {
		t.Parallel()
		response := "NEED_INFO Missing contracts:\n- UserRepository interface\n- DatabaseConfig type"
	...
	t.Run("prompt contains phase body and board snapshot", func(t *testing.T) {
		t.Parallel()
		inv := &mockInvoker{response: "PROCEED"}
		poller := &LLMPoller{Invoker: inv, Phases: phases}
		...
		if !strings.Contains(inv.captured, "Board State") {
			t.Error("prompt should contain board state")
		}
...
func TestParseResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			raw:          "PROCEED — All interfaces are available on the board.",
			wantDecision: PollProceed,
		},
		{
...
func TestBuildPollPrompt(t *testing.T) {
	...
	checks := []struct {
		...
		{"contains board state header", "## Board State"},
