...
func TestPollResultFields(t *testing.T) {
	...
	t.Run("proceed result", func(t *testing.T) {
		...
		r := PollResult{
			...
			Reason:   "all contracts available",
		}
...
func TestStateConstantsAlignWithPolling(t *testing.T) {
	t.Parallel()

	// Verify that the phase state constants used in polling are defined and
	// match expected string values for the board's state column.
...
func (m *mockPoller) Poll(_ context.Context, _ string, _ BoardSnapshot) (PollResult, error) {
	if m.err != nil {
