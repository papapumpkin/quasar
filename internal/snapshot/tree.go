// Package snapshot provides a project context scanner that generates deterministic
...
// It verifies that the field name is an exact match (not a prefix of a longer
// key) by checking that the character after the field name is whitespace or '='.
func extractTOMLField(content, field string) string {
	for _, line := range strings.Split(content, "\n") {
		...
		// Verify the field name is an exact match, not a prefix of a longer key.
