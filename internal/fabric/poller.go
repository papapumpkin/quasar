// Package fabric provides polling and blocked-phase tracking for the entanglement fabric.
//
// The Poller evaluates whether a phase has sufficient fabric context to proceed
// after its DAG dependencies are satisfied. This enables a checkpoint between
// "deps satisfied" and "start working" where the worker can inspect the fabric
...
const (
	// PollProceed means the fabric has everything the phase needs to start.
	PollProceed PollDecision = "PROCEED"

	// PollNeedInfo means the phase is missing entanglements it depends on
	...
	// with another phase's published entanglements or claims.
	PollConflict PollDecision = "CONFLICT"
)

// PollResult represents the outcome of a phase polling the fabric.
type PollResult struct {
	...
	// (e.g., entanglement names, type signatures, package paths).
	MissingInfo []string
...
// Poller evaluates whether a phase has sufficient fabric context to proceed.
// Implementations inspect the fabric snapshot and determine if the phase can
// start executing or needs to wait for more entanglements.
...
// and retries them when new entanglements appear on the fabric.
...
// BlockedTracker manages the set of phases that are waiting for fabric context.
