...
AgentMail is a shared mailbox MCP server that enables parallel quasar agents to coordinate via messages, advisory file claims, and change announcements. When running multiple coder-reviewer workers concurrently (e.g., `nebula apply --auto --max-workers 4`), agents use AgentMail to avoid file conflicts and stay aware of each other's changes.
...
A single AgentMail server is shared by all agents in a nebula run. It exposes 9 MCP tools over SSE/HTTP, with all coordination data persisted in a Dolt database for versioned history and audit trails.

### Prerequisites

- [Dolt](https://www.dolthub.com/docs/tutorials/installation/) — must be installed and running as a SQL server:

  ```bash
  dolt sql-server -u root -l 3306 --no-auto-commit
  ```

- The `agentmail` database must exist:

  ```bash
  dolt sql -q "CREATE DATABASE IF NOT EXISTS agentmail"
  ```
...
**Nebula `nebula.toml`** — enable per-nebula with optional port override:
...
When `agentmail` is enabled, Quasar automatically starts the AgentMail server process before launching workers and stops it when the nebula run completes.

### Quick Start

1. Install and start Dolt:

   ```bash
   dolt sql-server -u root -l 3306 --no-auto-commit
   ```

2. Build the agentmail binary:

   ```bash
   go build -o agentmail ./cmd/agentmail/
   ```

3. Enable agentmail in your nebula's `nebula.toml`:
...
4. Run the nebula:
   ...
   ```bash
   quasar nebula apply my-nebula/ --auto --max-workers 2
   ```

   Quasar will start the AgentMail server, wire it into each worker's Claude CLI invocation via `--mcp-config`, and shut it down when all tasks complete.

### MCP Tools

AgentMail provides 9 tools in four categories:

| Tool | Description |
|------|-------------|
| `register` | Register a new agent and receive an `agent_id` |
| `heartbeat` | Update agent liveness (stale agents are cleaned up after 2 min) |
| `send_message` | Send a message to a channel (default: `broadcast`) |
| `read_messages` | Read messages, optionally filtered by time and channel |
| `claim_files` | Claim advisory locks on files before modifying them |
| `release_files` | Release file claims when done |
| `get_file_claims` | Query which files are currently claimed and by whom |
| `announce_change` | Announce a file modification with a summary |
| `get_changes` | Query recent change announcements |

For full input/output schemas, example requests, and usage tips, see the [AgentMail Tool Reference](docs/agentmail.md).
